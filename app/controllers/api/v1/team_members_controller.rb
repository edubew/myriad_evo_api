module Api
  module V1
    class TeamMembersController < BaseController
      ...
      def index
        @members = policy_scope(TeamMember).order(:first_name)
        render json: {
          success: true,
      ...
      def create
        @member = current_company.team_members.build(
          member_params.merge(user: current_user)
        )
      ...
      def set_member
        @member = policy_scope(TeamMember).find(params[:id])
